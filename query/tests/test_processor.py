...
def test_processor_queues_extracted_listing_then_marks_email_read(tmp_path) -> None:
    ...
    staging_database = EmailDatabase(tmp_path / "staging.sqlite3")
    listings_database = ListingDatabase(tmp_path / "listings.sqlite3")
    ...
    assert listings_database.unread_listings() == []
...
def test_database_can_requeue_a_read_email_for_controlled_replay(tmp_path) -> None:
    ...
    database = EmailDatabase(tmp_path / "queue.sqlite3")
...
def test_processor_logs_and_marks_read_when_email_has_no_listing(caplog, tmp_path) -> None:
    ...
    with caplog.at_level("ERROR"):
        assert process_once(database, listings_database) == 1
