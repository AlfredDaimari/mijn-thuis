...
`query/` has three single-threaded Python processes and three operational SQLite databases. The Yahoo poller reads only messages sent from `stekkies.com` and writes the full email to `data/staging.sqlite3` with `is_read = 0`. The listing processor reads that staging queue, extracts HTTP(S) listing links and HTML anchor titles, writes each result to `data/listings.sqlite3` with its own `is_read = 0`, then marks the source email as read. The resolver reads that listing queue and writes the external provider URL to `data/resolved.sqlite3`, also unread. A SHA-256 signature prevents duplicate email queue entries, while `(source_signature, url)` prevents duplicate listing entries. The mailbox poller uses read-only IMAP and `BODY.PEEK[]`, so it does not modify Yahoo's read state. SQLite uses WAL mode and a busy timeout so the one poller and one processor can safely use separate connections; do not run multiple copies of a given consumer against the same database. All processes write timestamped logs without passwords or email bodies. The processor logs an error with the email subject when a Stekkies email has no extractable listing links.
